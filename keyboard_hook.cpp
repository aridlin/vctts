...
static LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
        ...
        k->vkCode == VK_MENU || k->vkCode == VK_LMENU || k->vkCode == VK_RMENU)
    {
        ...
        // While recording, swallow modifiers so game doesn't see them
    ...
    // Exit hotkey: Ctrl+Shift+Tab+E (on E-down)
    if (down && k->vkCode == 'E')
    {
    ...
    // Toggle: Ctrl+Backspace (on Backspace-down)
    if (down && k->vkCode == VK_BACK)
    {
        if (g_ctrl.load())
        {
            // Only allow toggle after config
    ...
    // If currently recording: swallow and build buffer
    if (g_state->recording.load())
    {
        if (down)
        {
            ...
            return 1; // swallow so games don't react
        }
